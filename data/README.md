...
| `eoportal_satellites_zh.jsonl` | JSON Lines | eoPortal 中文翻译（name_zh / summary_zh / faq_zh 等） | 首次发布（由 mimo-v2.5-pro 翻译） |
| `oscar_satellites.jsonl` | JSON Lines | OSCAR 1000+ 卫星列表（含 orbit/launch/agency/payload） | 首次发布 |
| `merged_index.json` | JSON | 双源去重合并的轻量索引（含 eoPortal detail 字段 + 中文翻译） | 首次发布 |
...
- `eoportal_satellites_zh.jsonl`：~1100 条 × ~2 KB（中文翻译）= ~2.2 MB
- `oscar_satellites.jsonl`：~1000 条 × ~0.5 KB = ~500 KB
- `merged_index.json`：~2000 条去重后 × ~3 KB = ~6 MB
